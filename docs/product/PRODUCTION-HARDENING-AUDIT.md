...
- `pnpm run check:secrets`: passed; scans tracked source/config for high-confidence secrets.
- `pnpm run check:production-env`: passed with a synthetic production-like env set; real Render/Vercel/Supabase values still need to pass in the deployment environment.
- `pnpm run build`: passed for `api-server`, `web-admin`, `web-student`, `web-sales`, `super-portal`, and `@repo/database`.
- `pnpm --filter super-portal test:e2e`: passed on Chromium.
- `pnpm test:e2e`: passed across `web-student`, `web-sales`, `web-admin`, and `super-portal` on Chromium.
...
| AUDIT-007 | Resolved | `web-student` dashboard E2E expected the authenticated dashboard, but `/en` could render the guest entry when server-side today fetch was unavailable. | Full E2E gate stopped before admin and super portal suites ran.                | Added a client authenticated fallback for `/en` that fetches today dashboard data when server fetch returns no initial data.     | `pnpm --filter web-student test:e2e -- --project=chromium`; `pnpm test:e2e`.                         |
| AUDIT-008 | Resolved | `web-student` roleplay E2E could not reliably submit text because the input/button state changed during submit.                                        | Roleplay text flow was hard to validate reliably.                              | Keep text until send succeeds, add an accessible send button label, and test the user keyboard-submit path.                      | Roleplay E2E passes without force-clicking or bypassing disabled state.                              |
| AUDIT-009 | Resolved | `web-admin` reports E2E still targeted a native select after the cohort filter moved to a searchable picker.                                           | Full E2E gate stopped in admin reports.                                        | Added listbox/option semantics to the picker and updated E2E to use the actual searchable cohort control.                        | `pnpm --filter web-admin test:e2e -- --project=chromium`; `pnpm test:e2e`.                           |
| AUDIT-010 | Medium   | Local release gate did not include a tracked-file secret scan.                                                                                         | Accidental keys in source/config could slip through local validation.          | Added `pnpm run check:secrets` and wired it into `release:check`.                                                                | `pnpm run check:secrets`.                                                                            |
