...
| Skill mastery / SRS         | Phần 1 đã làm  | Skill catalog + `SkillMastery` EWMA + admin/student UI + practice filter. SRS Core (`ReviewCard`, daily review) chưa làm.       |
...
1. `ReviewCard(userId, sourceType, sourceId, dueAt, interval, easeFactor, reps, lapses, lastReviewedAt)` — SM-2 scheduling cho SRS Core.
2. `AiConversation`, `AiMessage` (cho P8c roleplay).
3. `AiUsageQuota(tenantId, used, limit, resetAt)` — cho P8a/b/c quota management.
4. `MediaAsset(id, tenantId, type, url, uploadedBy, createdAt)` — cho P10 listening/audio.
5. `SkillMasteryHistory(userId, skillId, mastery, recordedAt)` — cho time-series chart sau này.
...
3. **NEXT**: SRS review queue MVP — daily review card trên dashboard (P9 core).
