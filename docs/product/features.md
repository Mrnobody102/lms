...
| Student dashboard  | MVP shell  | Continue learning + completion % đã có data nền activity, còn streak/chart UI |
...
1. `Unit` hoặc `Chapter`.
2. `Question`, `QuestionOption`, `ExerciseSet`, `PracticeAttempt`.
3. `Exam`, `ExamSection`, `ExamAttempt`, `ExamAnswer`.
4. `ActivationCode`, `Entitlement` hoặc `LicenseGrant`.
