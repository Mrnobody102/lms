...
All apps use `createApiClient` from `@repo/api-client` which handles token injection and tenant headers automatically:
...
The client reads the token from Zustand auth store and injects `Authorization` + `x-tenant-id` headers on every request. It also handles 401 responses by redirecting to login.
...
export class TenantMiddleware implements NestMiddleware {
  use(req: Request, res: Response, next: NextFunction) {
    ...
    const tenantId = req.headers['x-tenant-id'] || this.getTenantFromDomain(req); // Fallback: subdomain parsing
