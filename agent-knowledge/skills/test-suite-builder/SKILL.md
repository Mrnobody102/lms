...
- **auth_coverage**: Valid/invalid cookie or Bearer JWT, missing auth, wrong role, CSRF failures.
...
| ------------------------------------------------------- | --------------- |
| No auth cookie or Authorization header                  | 401             |
| Invalid token format or invalid auth cookie             | 401             |
| Valid token, wrong user role                            | 403             |
| Expired JWT token                                       | 401             |
| Valid token, correct role                               | 2xx             |
| Token from deleted user                                 | 401             |
| Missing tenant context on tenant-required public routes | 400             |
| Missing/invalid CSRF token on cookie-backed write route | 403             |
...
describe('AuthController', () => {
  describe('POST /auth/login', () => {
    it('should return 200 with user and auth cookies when credentials are valid');
...
| auth-standards   | Understanding cookie-backed browser auth, Bearer compatibility, and role-based auth |
