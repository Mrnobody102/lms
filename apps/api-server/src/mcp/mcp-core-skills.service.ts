...
export class McpCoreSkillsService {
  ...
  @McpTool({
    ...
    description: 'Hiển thị cấu trúc thư mục của dự án LMS để AI Agent hiểu ngữ cảnh mã nguồn.',
    schema: z.object({
      path: z.string().optional().describe('Đường dẫn thư mục cần kiểm tra (mặc định là root)'),
    }),
  })
  ...
  @McpTool({
    name: 'course_search',
    description: 'Tìm kiếm các khóa học trong hệ thống LMS từ Database thực tế.',
    schema: z.object({
      keyword: z.string().describe('Từ khóa tìm kiếm tiêu đề khóa học (ví dụ: "AI", "React")'),
    }),
  })
  ...
  @McpTool({
    ...
    description: 'Đọc nội dung của một file cụ thể trong dự án để phân tích code.',
    schema: z.object({
