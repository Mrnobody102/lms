...
interface RoleplayManagerProps {
...
export function RoleplayManager({
  ...
  // Standalone mode: auto-select the first course so content shows immediately.
  useEffect(() => {
    if (showCourseSelector && !draft.courseId && courses[0]?.id) {
      setDraft((current) => ({ ...current, courseId: courses[0].id }));
    }
  }, [courses, draft.courseId, showCourseSelector]);

  // Embedded mode: keep the draft locked to the host course.
  useEffect(() => {
    if (lockedCourseId && draft.courseId !== lockedCourseId) {
      setDraft((current) => ({ ...current, courseId: lockedCourseId }));
    }
  }, [lockedCourseId, draft.courseId]);
        ...
        <form className="space-y-4 rounded-lg border bg-card p-4" onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <TextField
              ...
              onChange={(targetLanguage) => setDraft((current) => ({ ...current, targetLanguage }))}
            />
            <div className="space-y-1.5">
          ...
          <div className="flex gap-2">
            ...
            {editingId ? (
              <Button type="button" variant="outline" onClick={() => resetDraft(draft.courseId)}>
                {t('cancel')}
              </Button>
            ) : null}
          </div>
                  ...
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div>
                      <div className="mb-2 flex flex-wrap items-center gap-2">
                        ...
                        <Badge variant="outline">{t(`roleplayModeValue.${scenario.mode}`)}</Badge>
                      </div>
